...
def main():
    ...
    parser.add_option("-o", "--dfile",
                  dest = "dpath",
                  help = "Path to runs files stats", 
                  )
    parser.add_option("-S", "--step", dest="step", type="int",
                      help="step for stats", default=1000)
    parser.add_option("-r", "--retrieve", dest="retrieve",action="store_true",
                      help="Retrieve list docs", default=False)
    ...
    # Retrieve Liste documents based on INEX
    if options.retrieve: 
        if options.dpath and options.topics:
            RunManager(options= options, index=index, docCount=doc_count, docList=_docIdList).run()    
        else:
